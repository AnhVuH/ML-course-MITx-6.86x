...
def project_onto_PC(X, pcs, n_components):
    ...
    center_matrix = center_data(X)
    first_n_components = pcs[:,0:n_components]
    return np.matmul(center_matrix, first_n_components)
...
def plot_PC(X, pcs, labels):
    ...
    ax.set_xlabel('PC 1')
    ax.set_ylabel('PC 2')
    # plt.savefig("plot.png") # to run on google colab
    plt.show()
