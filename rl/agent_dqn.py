...
def epsilon_greedy(state_vector, epsilon):
    ...
    # print(q_values_action)
    # print(q_values_object)
    exploitation_action_index = q_values_action.argmax()
    exploitation_object_index = q_values_object.argmax()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_values_action_cur, q_values_object_cur =q_value_cur_state
    q_value = 1/2*(q_values_action_cur[action_index]+ q_values_object_cur[object_index])


    # TODO Your code here
    loss = 1/2 *((reward + GAMMA * maxq_next)-q_value)**2
    if not terminal:
        q_values_action_cur[action_index] =(1-ALPHA)* q_values_action_cur[action_index] + ALPHA *(reward + GAMMA * maxq_next)
        q_values_object_cur[action_index] =(1-ALPHA)* q_values_object_cur[action_index] + ALPHA *(reward + GAMMA * maxq_next)
    else:
        q_values_action_cur[action_index] = (1 - ALPHA) * q_values_action_cur[action_index] + ALPHA * reward
        q_values_object_cur[action_index] = (1 - ALPHA) * q_values_object_cur[action_index] + ALPHA * reward
